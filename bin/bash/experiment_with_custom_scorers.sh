...
variable_exists () {
    ...
    if [ ! -n "$VARIABLE" ]; then
        echo "[unix] variable=$VARIABLE_NAME does not exists. Usage : bin/bash/experiment_with_custom_scorers.sh <path_to_experiment_cfg> <path_to_experiment_directory>. Exiting with status code 1"
        exit 1
    fi
}
...
file_exists () {
    local FILE=$1
    if [ ! -e $FILE ]; then
        echo "[unix] file=$FILE does not exist. Usage : bin/bash/experiment_with_custom_scorers.sh <path_to_experiment_cfg> <path_to_experiment_directory>"
        exit 1
    fi
}
...
directory_exists () {
    local DIR_NAME=$1
    if [ ! -d "$DIR_NAME" ]; then
        echo "[unix] directory=$DIR_NAME does not exist. Usage : bin/bash/experiment_with_custom_scorers.sh <path_to_experiment_cfg> <path_to_experiment_directory>"
        exit 1
    fi
}
...
# Preliminaries
echo "[unix] Starting script experiment.sh using custom scorers..."
set -e
...
TEST_SCRIPT=$PYTHON_DIRECTORY/testproxy.py
...
RR_EXPERIMENT_FILE=$DATA_DIRECTORY/exp_retrieve_and_rank_scorers.json
echo "-----------------------------------"
...
python $TEST_SCRIPT --username=$RETRIEVE_AND_RANK_USERNAME --password=$RETRIEVE_AND_RANK_PASSWORD --url=$RETRIEVE_AND_RANK_BASE_URL \
    ...
    --output-file=$RR_EXPERIMENT_FILE --fl=id,title,subtitle,answer,answerScore,accepted,upModVotes,downModVotes --debug \
    --ranker-id=$RANKER_ID
